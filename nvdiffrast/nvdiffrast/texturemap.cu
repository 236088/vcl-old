...
__global__ void downSampling(const TexturemapParams tp, int index, int width, int height) {
	...
	int p00idx = px * 2 + width * 2 * (py * 2 + height * 2 * pz * 2);
	int p01idx = (px * 2 + 1) + width * 2 * (py * 2 + height * 2 * pz * 2);
	int p10idx = px * 2 + width * 2 * ((py * 2 + 1) + height * 2 * pz * 2);
	int p11idx = (px * 2 + 1) + width * 2 * ((py * 2 + 1) + height * 2 * pz * 2);
...
__device__ __forceinline__ int4 indexFetch(const TexturemapParams tp, int level, float2 uv, float2& t) {
	...
	int u0 = t.x<0 ? 0 : t.x>size.x - 1 ? (int)size.x - 1 : (int)t.x;
	int u1 = t.x<1 ? 0 : t.x>size.x - 2 ? (int)size.x - 1 : (int)t.x + 1;
	int v0 = t.y<0 ? 0 : t.y>size.y - 1 ? (int)size.y - 1 : (int)t.y;
	int v1 = t.y<1 ? 0 : t.y>size.y - 2 ? (int)size.y - 1 : (int)t.y + 1;
	int4 idx;
	idx.x = v0 * size.x + u0;
	idx.y = v0 * size.x + u1;
	idx.z = v1 * size.x + u0;
	idx.w = v1 * size.x + u1;
	t.x = (t.x<0 || t.x>size.x) ? 0 : t.x - floor(t.x);
	t.y = (t.y<0 || t.y>size.x) ? 0 : t.y - floor(t.y);
	return idx;
}
...
__global__ void texturemapFowardKernel(const TexturemapParams tp, const RenderingParams p) {
	...
	int level0 = level <= 0 ? 0 : tp.miplevel - 1 <= level ? tp.miplevel - 1 : (int)floor(level);
	int level1 = level <= 0 ? 0 : tp.miplevel - 1 <= level ? tp.miplevel - 1 : (int)ceil(level);
...
__device__ __forceinline__ void calculateLevel(const TexturemapParams tp, int pidx, int& level0, int& level1, float& flevel, float4& dleveldda) {
	...
	float c2 = b * b - a * a;
	float c = sqrt(c2);
	float d = 0.72134752f / (b * c + c2);
	float bc = b + c;

	float level = 0.5 * log2f(b + c);
	level0 = level <= 0 ? 0 : tp.miplevel - 1 <= level ? tp.miplevel - 1 : (int)floor(level);
	level1 = level <= 0 ? 0 : tp.miplevel - 1 <= level ? tp.miplevel - 1 : (int)ceil(level);
...
__global__ void texturemapBackwardKernel(const TexturemapParams tp, const RenderingParams p) {
	...
	for (int i = 0; i < tp.channel; i++) {
		...
		if (flevel > 0) {
			...
			t00 = lerp(t00, tp.miptex[level1][idx1.x * tp.channel + i], flevel);
			t01 = lerp(t01, tp.miptex[level1][idx1.y * tp.channel + i], flevel);
			t10 = lerp(t10, tp.miptex[level1][idx1.z * tp.channel + i], flevel);
			t11 = lerp(t11, tp.miptex[level1][idx1.w * tp.channel + i], flevel);
...
__global__ void texturemapGradSum(const TexturemapParams tp) {
	...
	for (int l = 0; l < tp.miplevel; l++) {
		int p = px + w * (py + h * pz);
		for (int i = 0; i < tp.channel; i++) {
			tp.gradTex[pidx * tp.channel + i] += tp.gradMipTex[l][p * tp.channel + i];
		}
		px >>= 1; py >>= 1;
		w >>= 1; h >>= 1;
	}
