...
void PresetFilter::init(int resolution, int count) {
	...
	Filter::init(predict_fp, predict_rp, predict_ap.kernel.out, 3, count);
	...
	Filter::init(target_fp, target_rp, target_ap.kernel.out, 3, count);

	Loss::init(loss, predict_fp.kernel.out, target_fp.kernel.out, 512, 512, 3);
	Filter::init(predict_fp, loss.grad);
	Antialias::init(predict_ap, predict_rp, predict_fp.grad.in);
	...
	GLbuffer::init(target_buffer, target_fp.kernel.out, resolution, resolution, 3, 15);
	GLbuffer::init(predict_buffer, predict_fp.kernel.out, resolution, resolution, 3, 14);
