...
void PresetFilter::init(int resolution, int count) {
	...
	Filter::init(target_fp, p, target.ap.kernel.out, 3, count);
	predict.init(p, mat, predict_pos, predict_color);
	Filter::init(predict_fp, p, predict.ap.kernel.out, 3, count);

	Loss::init(loss, predict_fp.kernel.out, target_fp.kernel.out, p, 3);
	Filter::init(predict_fp, p, loss.grad);
	Antialias::init(predict.ap, p, predict.rp, predict_fp.grad.in);
	...
	GLbuffer::init(target_buffer, target_fp.kernel.out, p.width, p.height, 3, 15);
	GLbuffer::init(predict_buffer, predict_fp.kernel.out, p.width, p.height, 3, 14);
	GLbuffer::init(hr_target_buffer, hr_target.ap.kernel.out, 512, 512, 3, 13);
	GLbuffer::init(hr_predict_buffer, hr_predict.ap.kernel.out, 512, 512, 3, 12);
}

void PresetFilter::display(void) {
	Matrix::forward(mat);
	target.forward();
	Filter::forward(target_fp);
	predict.forward();
	Filter::forward(predict_fp);

	Loss::backward(loss);
	Filter::backward(predict_fp);
	...
	GLbuffer::draw(target_buffer, GL_RGB32F, GL_RGB, 0.f, 0.f, 1.f, 1.f);
	GLbuffer::draw(predict_buffer, GL_RGB32F, GL_RGB, -1.f, 0.f, 0.f, 1.f);
	GLbuffer::draw(hr_predict_buffer, GL_RGB32F, GL_RGB, -1.f, -1.f, 0.f, 0.f);
	GLbuffer::draw(hr_target_buffer, GL_RGB32F, GL_RGB, 0.f, -1.f, 1.f, 0.f);
	glFlush();
}
