...
void Matrix::forward(Matrix& mat) {
	...
	cudaMemcpy(mat.r, &mat, 16 * sizeof(float), cudaMemcpyHostToDevice);
	matrix = glm::translate(mat.translation) * matrix;
	cudaMemcpy(mat.m, &mat, 16 * sizeof(float), cudaMemcpyHostToDevice);
	matrix = glm::lookAt(mat.eye, mat.origin, mat.up) * matrix;
	cudaMemcpy(mat.mv, &mat, 16 * sizeof(float), cudaMemcpyHostToDevice);
	matrix = glm::perspective(glm::radians(mat.fovy), mat.aspect, mat.znear, mat.zfar) * matrix;
	cudaMemcpy(mat.mvp, &mat, 16 * sizeof(float), cudaMemcpyHostToDevice);
}
