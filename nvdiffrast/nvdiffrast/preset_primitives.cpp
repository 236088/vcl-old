...
void PresetPrimitives::init() {
	...
	GLbuffer::init(ip_buffer, ip.kernel.out, width, height, 2, 14);
	GLbuffer::init(pos_ip_buffer, pos_ip.kernel.out, width, height, 3, 13);
	GLbuffer::init(norm_ip_buffer, norm_ip.kernel.out, width, height, 3, 12);
	GLbuffer::init(tp_buffer, tp.kernel.out, width, height, 3, 11);
	GLbuffer::init(mp_buffer, mp.kernel.out, width, height, 3, 10);
	GLbuffer::init(ap_buffer, ap.kernel.out, width, height, 3, 9);
	GLbuffer::init(fp_buffer, fp.kernel.out, width, height, 3, 8);
}
